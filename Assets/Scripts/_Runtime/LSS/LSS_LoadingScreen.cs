...
namespace Michsky.LSS
{
	...
	public class LSS_LoadingScreen : MonoBehaviour
	{
		...
		{
			LoadScene(SceneManager.LoadSceneAsync(targetScene), targetScene);
		}

		public static void LoadScene(int targetScene)
		{
			LoadScene(SceneManager.LoadSceneAsync(targetScene), SceneManager.GetSceneAt(targetScene).name);
		}
		...
		public static void LoadSceneAdditive(int targetScene, string targetPrefab)
		{
			presetName = targetPrefab;
			LoadSceneAdditive(targetScene);
		}
		...
		{
			LoadSceneAdd(SceneManager.LoadSceneAsync(targetScene, LoadSceneMode.Additive), targetScene);
		}

		public static void LoadSceneAdditive(int targetScene)
		{
			LoadSceneAdd(SceneManager.LoadSceneAsync(targetScene, LoadSceneMode.Additive), SceneManager.GetSceneAt(targetScene).name);
		}

		static void LoadScene(AsyncOperation asyncOperation, string targetScene)
		{
			try
			{
				...
				instance.loadingProcess = asyncOperation;
		...
		static void LoadSceneAdd(AsyncOperation asyncOperation, string targetScene)
		{
			try
			{
				instance = Instantiate(Resources.Load<GameObject>("Loading Screens/" + presetName).GetComponent<LSS_LoadingScreen>());
